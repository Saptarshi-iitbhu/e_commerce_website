...
const CartTotal = ({method, setMethod}) => {
  ...
  return (
    <div>
      ...
      <hr className='border-gray-30 *:my-5'/>

      {/* PAYMENT METHOD */}
      {isOrderPage && (
        <div className='mb-6'>
            <div className='my-6'>
                <h4 className='h4 mb-5'>Payment <span>Method</span></h4>
      ...
      <div className='mt-4 space-y-2'>
        ...
        <div className='flex justify-between'>
            ...
            <p className='font-bold'>{getCartAmount() === 0 ? "$0.00" : `${currency}${delivery_Charges}.00`}</p>
        </div>
        <div className='flex justify-between'>
            <h5 className='h5'>Tax (2%)</h5>
            <p className='font-bold'>{currency}{(getCartAmount()*2)/100}</p>
        </div>
        <div className='flex justify-between text-lg font-medium mt-3'></div>
