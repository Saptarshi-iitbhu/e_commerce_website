  ...
  return (
    <div className='overflow-hidden p-5 bg-white'>
      ...
      className='flexCenter bg-{#f5f5f5} overflow-hidden relative'>
        <img src={product.image.length > 1 && hovered ? product.image[1] : product.image[0]} alt="productImg" 
      ...
      <div className='p-3'>
        ...
        <div className='flexBetween pt-2 gap-2'>
          ...
          <button onClick={()=>addToCart(product._id)} className='btn-outline !py-2 !px-0 w-full !text-xs'>Add to Cart | ${product.offerPrice}.00</button>
        </div>
      </div>
    </div>
  )
