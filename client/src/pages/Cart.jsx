...
const Cart = () => {
  ...
  const { navigate, products, currency, cartItems, updateQuatity} = useContext(ShopContext)
  ...
  const increment = (id, size)=>{
    ...
    updateQuatity(id, size, currentQuantity + 1)
  }
      ...
      <div className='flex flex-col xl:flex-row gap-20 xl:gap-28'>
            ...
            return (
              <div key={i} className='grid grid-cols-[6fr_1fr_1fr] items-center bg-white p-2'>
                <div className='flex items-center md:gap-6 gap-3'>
                  <div className='flex bg-primary'><img src={product.image[0]} alt='productImg' className='w-20'></img></div>
                  <div className=''>
                    <h5 className='hidden sm:block h5 line-clamp-1'>{product.name}</h5>
                ...
                <p className='text-center'>{currency}{product.offerPrice * quantity}</p>
                <button onClick={()=>updateQuatity(item._id, item.size, 0)} className='cursor-pointer mx-auto'>
        ...
        <div className='flex-1 flex-col'>
          <div className='max-w-[360px] w-full bg-white p-5 py-10 max-md:mt-16'>
            <CartTotal />
            <button onClick={()=>navigate('./place-order')} className='btn-dark w-full mt-8'>
              Proceed to Delivery
            </button>
          </div>
        </div>
