      ...
      {orders.map((order)=>(
        <div key={order._id} className='bg-white p-2 mt-3 rounded-sm'>
            ...
            <div key={idx} className='text-gray-700 flex flex-col lg:flex-row gap-4 mb-3'>
              <div className='flex flex-[2] gap-x-3'>
                <div className='flexCenter bg-primary'>
                  <img src={item.product.image[0]} alt="orderImg" className='max-h-20 max-w-20 object-contain'/>
                </div>
          ...
          <div className='flex gap-3'>
            ...
            <button onClick={loadOrderData} className='btn-secondary !py-1 !text-sm rounded-sm'>Track Order</button>
          </div>
          </div>
        </div>
      ))}     
